...
public class TrophyData2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        currentValue.text = PlayerPrefs.GetInt("trophy2_currentValue").ToString();
        Debug.Log("Current=" + PlayerPrefs.GetInt("trophy2_currentValue").ToString());
    ...
    void addExp()
    {
        if (PlayerPrefs.GetInt("trophy2_currentValue") < int.Parse(maxValue.text))
        {
            PlayerPrefs.SetInt("trophy2_currentValue", PlayerPrefs.GetInt("total_organic_collected"));

            if (PlayerPrefs.GetInt("trophy2_currentValue") >= int.Parse(maxValue.text))
            {
                if (PlayerPrefs.GetInt("trophy2_done") == 0)
                {
                    accomplish();
                    PlayerPrefs.SetInt("trophy2_done", 1);
                }
                PlayerPrefs.SetInt("trophy2_currentValue", int.Parse(maxValue.text));
            }
            else if ((PlayerPrefs.GetInt("trophy2_currentValue") >= int.Parse(thirdGoal.text)) && PlayerPrefs.GetInt("trophy2_goal3") == 0)
            {
                // gems
                PlayerPrefs.SetInt("gems", PlayerPrefs.GetInt("gems") + int.Parse(thirdReward.text));
                PlayerPrefs.SetInt("trophy2_goal3", 1);
            }
            else if ((PlayerPrefs.GetInt("trophy2_currentValue") >= int.Parse(secondGoal.text)) && PlayerPrefs.GetInt("trophy2_goal2") == 0)
            {
                // coins
                PlayerPrefs.SetInt("coins", PlayerPrefs.GetInt("coins") + int.Parse(secondReward.text));
                PlayerPrefs.SetInt("trophy2_goal2", 1);
            }
            else if ((PlayerPrefs.GetInt("trophy2_currentValue") >= int.Parse(firstGoal.text)) && PlayerPrefs.GetInt("trophy2_goal1") == 0)
            {
                // exp
                PlayerPrefs.SetInt("exp", PlayerPrefs.GetInt("exp") + int.Parse(firstReward.text));
                PlayerPrefs.SetInt("trophy2_goal1", 1);
            }
    ...
    float CalculateExp()
    {
        return PlayerPrefs.GetInt("trophy2_currentValue") / float.Parse(maxValue.text);
    }
